...
async def main(broker_host, token):
    ...
    client.publish('TEST/TIME', 'Retained: {}'.format(time.time()), retain=True)
    client.publish('TEST/TIME', 'Not retained: {}'.format(time.time()), retain=False)

    await client.disconnect()
    # after connect client will receive retained message
    await client.connect(broker_host)
